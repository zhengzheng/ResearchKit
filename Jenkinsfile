    ...
    stages {
            ...
            stage('Build (ORKTest iOS)') {
                ...
                sh 'echo "Build (ORKTest iOS)"'
                                    sh 'set -o pipefail && xcodebuild clean build-for-testing -project ./Testing/ORKTest/ORKTest.xcodeproj -scheme "ORKTest" -destination "name=iPhone Xs" | tee output/ORKTest/ios/build.log | /usr/local/bin/xcpretty'
                            }
                    }
            }
            stage('Test (ORKTest iOS)') {
                ...
                sh 'echo "Test (ORKTest iOS)"'
                                    sh 'set -o pipefail && xcodebuild test-without-building -project ./Testing/ORKTest/ORKTest.xcodeproj -scheme "ORKTest" -destination "name=iPhone Xs" | tee output/ORKTest/ios/test.log | /usr/local/bin/xcpretty -r junit'
                            }
                    }
            }
    }
